    ...
    **/
    ...
    function convertTo($filename=string) {
        if($filename) {
            ...
            file_put_contents('build/js/image.data.js', 'jQuery.UUIBase.data["' . urlencode($filename) . '"]="data:image\/' . $filetype . ';base64, ' . preg_replace('/\//', '\/', base64_encode($imgbinary)) . '";', FILE_APPEND);
        }
    };
    /**
     create a ui
                    ...
                    if($all) {
                        ...
                        foreach($allurl as $key=>$value) {
                            if($value[0]) {
                                ...
                                $mobilecss = preg_replace('/' . preg_replace(array('/\//', '/\(/', '/\)/'), array('\/', '\(', '\)'), $value[0]) . '/', 'url("+jQuery.UUIBase.data["' . urlencode($hash) . '"]+")', $mobilecss);
                            }
                        }
                    }
                    $mobilecss = 'jQuery.UUIBase.css.push("' . $mobilecss.'");';
                }
